...
class CVAE:
    def __init__(self, num_users, num_items, num_factors, params, input_dim,
        ...
        print("only initializing x_recon")
    ...
    def transform(self, data_x):
        # This transforms the items.
        data_en = self.sess.run(self.z_mean, feed_dict={self.x: data_x})
        return data_en

    def predict_transform(self, user_vector, items, params):
        """
        users: list of list
        """
        ...
        if n > 0:
            ...
            return scipy.linalg.solve(A, x).reshape(1, self.n_z)
        else:
            return np.array([])
    ...
    def evaluateCorrect(self, train_users, test_users, M):
        m_U = self.m_U
        m_V = self.m_V
        ...
        batch_size = 100
        n = int(math.ceil(1.0 * m_num_users / batch_size))
        num_hit = np.zeros([m_num_users, M])
        num_total = np.zeros([m_num_users, 1])

        for i in range(1, n):
            print("Processing batch {} of {}".format(i, n))
            offset = (i - 1) * batch_size
            index_range = np.arange(offset, min(i*batch_size, m_num_users))

            score = np.dot(m_U[index_range, :], m_V.transpose())

            gap = np.max(score) - np.min(score)
            batch_size = len(index_range)

            ground_truth = np.zeros([batch_size, m_num_items])

            for j in index_range:
                current_user = train_users[offset + j]
                ground_truth[j, current_user[:]] = 1

            score = score - ground_truth * gap
            I = np.argsort(score, axis=1)[:, ::-1]

            ground_truth = np.zeros([batch_size, m_num_items])

            for j in index_range:
                current_user = test_users[offset+j]
                ground_truth[j, current_user[:]] = 1

            recommendation = np.zeros([batch_size, m_num_items])

            for j in index_range:
                recommendation[j, :] = ground_truth[j, I[j, :]]

            num_hit[offset + index_range, :] = recommendation[:, range(M)]
            num_total[offset + index_range, :] = np.sum(recommendation, axis=1).reshape(len(index_range), 1)

        return np.mean(np.cumsum(num_hit, axis=1) / np.tile(num_total, M), axis=0)
    ...
    def run(self, users, items, test_users, test_items, data_x, params):
        ...
        for epoch in range(params.n_epochs):
            ...
            self.m_theta[:] = self.transform(data_x)
            likelihood = self.pmf_estimate(users, items, params)
